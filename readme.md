...
1.  **Open a Cheat File:**
    *   **Drag and Drop:** The easiest way is to drag your cheat file and drop it onto the **Assembler Panel** (left) or **Opcodes Panel** (center).
    *   **Open File Button:** Click "Open ASM File" to load your cheat source code, or "Open Opcodes File" to load raw opcodes.
2.  **Write or Edit a Cheat:** In the **Assembler Panel** (left), write a new cheat or modify an existing one. As you type, the **Opcodes Panel** (center) will update with the raw opcodes.
3.  **Disassemble a Cheat:** Paste a raw cheat code into the **Opcodes Panel** (center). The **Disassembly Panel** (right) will automatically show you the human-readable version.
...
## The User Interface: Assembler, Opcodes, and Disassembly Panels

The Cheat Code Commander interface is designed for a clear and organized workflow around three core panels.

-   **Assembler Panel (Left):** This is where you write and edit your cheats using the custom, human-readable syntax. The assembler provides real-time feedback, highlighting errors and showing the assembled opcodes in the **Opcodes Panel**.
-   **Opcodes Panel (Center):** This shared, editable text area displays the raw hexadecimal opcodes generated by the assembler. You can also paste raw cheats here to have them disassembled in the **Disassembly Panel**.
-   **Disassembly Panel (Right):** This panel shows the human-readable output of the disassembler. It converts raw opcodes from the **Opcodes Panel** into a more understandable format, including ARM64 instructions where possible.
...
*   **Layout:** When enabled, the application displays two **Opcodes Panels** in the center.
*   **Independent Updates:** The center-left **Opcodes Panel** updates with your live assembly changes, while the center-right **Opcodes Panel** can be used to load a separate opcode file for reference. Both panels feed into the **Disassembly Panel**.
*   **Copy to Right:** A context menu option allows you to quickly copy opcodes from the left **Opcodes Panel** to the right, setting a new baseline for comparison.
...
The assembler works in real-time. As you type in the **Assembler Panel** (left), the **Opcodes Panel** (center) instantly updates to show the assembled opcodes. If there are any syntax errors in your code, the assembler will highlight the problematic line to help you correct it.
...
## The Disassembly Panel: Understanding the Code

The **Disassembly Panel** is your window into the inner workings of a cheat. It takes raw hexadecimal opcodes from the **Opcodes Panel** and converts them into a human-readable format.

### Live Disassembly

The disassembler works in real-time. Paste any raw cheat code into the **Opcodes Panel** (center), and the **Disassembly Panel** (right) will instantly show you the disassembled output. This is incredibly useful for understanding, debugging, or modifying a cheat.
...
1.  **Combine Opcodes:** Copy the raw opcodes from all your different cheat files and paste them together into the **Opcodes Panel** (center).
2.  **Disassemble:** The tool will automatically disassemble the combined opcodes in the **Disassembly Panel** (right).
3.  **Copy to Assembler:** Right-click anywhere in the **Disassembly Panel** and select **"Copy to Assembler Panel"**. This will recreate the entire editable cheat source in the **Assembler Panel**, which is the primary method for modifying cheats when you don't have the original source code.
